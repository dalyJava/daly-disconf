...
public class ConfigConsistencyMonitorServiceImpl implements IConfigConsistencyMonitorService {
    ...
    private void checkAppEnvVersionConfigConsistency(App app, Env env, String version) {
        ...
        //
        //
        List<Config> configList = configMgr.getConfigList(config, true, true);

        // 准备发送邮件通知
        String toEmails = appMgr.getEmails(app.getId());
        List<ConfListVo> confListVos = new ArrayList<>();
        configList.stream().forEach(configTemp -> {
            ConfListVo confListVo = new ConfListVo();
            confListVo.setAppId(configTemp.getAppId());
            confListVo.setEnvId(configTemp.getEnvId());
            confListVo.setValue(configTemp.getValue());
            confListVo.setVersion(configTemp.getVersion());
            confListVo.setType(String.valueOf(configTemp.getType()));
            confListVo.setCreateTime( configTemp.getCreateTime());
            confListVos.add(confListVo);
        });
